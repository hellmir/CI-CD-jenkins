            ...
            steps {
                script {
                    withCredentials([
                        ...
                        '''
                        sh '''
                        ssh -i "$EC2_DEPLOY_KEY" ubuntu@"$EC2_IP" "export DB_ROOT_PASSWORD='$DB_ROOT_PASSWORD'; export DB_USER_NAME='$DB_USER_NAME'; export DB_USER_PASSWORD='$DB_USER_PASSWORD'; export PROJECT_NAME='$PROJECT_NAME'; chmod +x ${EC2_DEPLOY_PATH}/set-up-docker.sh && ${EC2_DEPLOY_PATH}/set-up-docker.sh"
                        '''
                        sh '''
